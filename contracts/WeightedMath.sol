...
import "./math/MathForWeighted.sol";
...
library WeightedMath {
    ...
    function _calcInGivenOut(
        ...
        **********************************************************************************************/
        ...
        uint256 ratio = power.sub(FixedPoint.getONE());
    ...
    function _calcBptOutGivenExactTokensIn(
        ...
        uint256 bptOut = (invariantRatio > FixedPoint.getONE())
            ? bptTotalSupply.mulDown(invariantRatio - FixedPoint.getONE())
            : 0;
        return bptOut;
    }
    ...
    function _calcBptOutGivenExactTokenIn(
        ...
        uint256 amountInWithoutFee;
        {
            ...
            // The use of `normalizedWeight.complement()` assumes that the sum of all weights equals FixedPoint.getONE().
            ...
            if (balanceRatioWithFee > invariantRatioWithFees) {
                uint256 nonTaxableAmount = invariantRatioWithFees > FixedPoint.getONE()
                    ? balance.mulDown(invariantRatioWithFees - FixedPoint.getONE())
                    : 0;
    ...
    function _computeJoinExactTokensInInvariantRatio(
        ...
        // increase.
        invariantRatio = FixedPoint.getONE();

        for (uint256 i = 0; i < balances.length; i++) {
            uint256 amountInWithoutFee;

            if (balanceRatiosWithFee[i] > invariantRatioWithFees) {
                // invariantRatioWithFees might be less than FixedPoint.getONE() in edge scenarios due to rounding error,
                // particularly if the weights don't exactly add up to 100%.
                uint256 nonTaxableAmount = invariantRatioWithFees > FixedPoint.getONE()
                    ? balances[i].mulDown(invariantRatioWithFees - FixedPoint.getONE())
                    : 0;
                ...
                // Pool with only a subset of tokens). In this case, `balanceRatio` will equal `FixedPoint.getONE()`, and
    ...
    function _calcTokenInGivenExactBptOut(
        ...
        ******************************************************************************************/
        ...
        require(invariantRatio <= _MAX_INVARIANT_RATIO, "Errors.MAX_OUT_BPT_FOR_TOKEN_IN");

        // Calculate by how much the token balance has to increase to match the invariantRatio
        uint256 balanceRatio = invariantRatio.powUp(FixedPoint.getONE().divUp(normalizedWeight));

        uint256 amountInWithoutFee = balance.mulUp(balanceRatio.sub(FixedPoint.getONE()));
    ...
    function _calcTokenOutGivenExactBptIn(
        ...
        *****************************************************************************************/
        ...
        require(invariantRatio >= _MIN_INVARIANT_RATIO, "Errors.MIN_BPT_IN_FOR_TOKEN_OUT");

        // Calculate by how much the token balance has to decrease to match invariantRatio
        uint256 balanceRatio = invariantRatio.powUp(FixedPoint.getONE().divDown(normalizedWeight));
    ...
    function _calcBptOutAddToken(uint256 totalSupply, uint256 normalizedWeight) internal pure returns (uint256) {
        ...
        uint256 weightSumRatio = FixedPoint.getONE().divDown(FixedPoint.getONE().sub(normalizedWeight));

        // The amount of BPT to mint is then simply:
        //
        // toMint = totalSupply * (weightSumRatio - 1)

        return totalSupply.mulDown(weightSumRatio.sub(FixedPoint.getONE()));
    }
